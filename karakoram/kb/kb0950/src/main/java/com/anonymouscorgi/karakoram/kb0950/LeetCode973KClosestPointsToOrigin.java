...
interface LeetCode973KClosestPointsToOrigin {
  ...
  LeetCode973KClosestPointsToOrigin Method = new LeetCode973KClosestPointsToOrigin() {
    @Override
    public int[][] kClosest(int[][] points, int k) {
      Object[][] objPoints = new Object[points.length][];
      for (int i = 0; i < points.length; i++) {
        objPoints[i] = new Object[]{points[i][0] * points[i][0] + points[i][1] * points[i][1],
            points[i]};
      }
    ...
    private static Object[] kClosest(Object[][] points, int start, int end, int k) {
    ...
    private static void swap(Object[][] points, int a, int b) {
      if (a != b) {
